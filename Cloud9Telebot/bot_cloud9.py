...
def start(bot, update):
    ...
    '''*Добро пожаловать в наш ресторан!* Чтобы оформить заказ, наберите /order''', parse_mode='Markdown')
...
def add_dishes(bot, id, dish):
    conn1 = sqlite3.connect('Restaurant.db')
    c1 = conn1.cursor()
    c1.execute("INSERT INTO Заказы_Блюда (Заказ, Блюдо) VALUES ('%s','%s')" % (ordername[id], dish))
    conn1.commit()
    c1.close()
    conn1.close()
    bot.sendMessage(chat_id=id, text='Введите блюдо. Когда закончите, введите /end')
